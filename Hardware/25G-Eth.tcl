...
# configure the cores
if {$QSFP_SPEED == "10G"} {
	set_property -dict [list CONFIG.ETHERNET_BOARD_INTERFACE $QSFP_MODE CONFIG.DIFFCLK_BOARD_INTERFACE $QSFP_CLOCK CONFIG.USE_BOARD_FLOW true CONFIG.LINE_RATE {10} CONFIG.INCLUDE_AXI4_INTERFACE {1} CONFIG.ENABLE_TX_FLOW_CONTROL_LOGIC {0} CONFIG.ENABLE_RX_FLOW_CONTROL_LOGIC {0}] [get_bd_cells QSFP/QSFP_${QSFP_INDEX}/eth]
} elseif {$QSFP_SPEED == "25G"} {
	set_property -dict [list CONFIG.ETHERNET_BOARD_INTERFACE $QSFP_MODE CONFIG.DIFFCLK_BOARD_INTERFACE $QSFP_CLOCK CONFIG.USE_BOARD_FLOW true CONFIG.LINE_RATE {25} CONFIG.INCLUDE_AXI4_INTERFACE {1} CONFIG.GT_DRP_CLK {200} CONFIG.ENABLE_TX_FLOW_CONTROL_LOGIC {0} CONFIG.ENABLE_RX_FLOW_CONTROL_LOGIC {0}] [get_bd_cells QSFP/QSFP_${QSFP_INDEX}/eth]
} else {
...
set_property -dict [list CONFIG.S_TDATA_NUM_BYTES {64} CONFIG.M_TDATA_NUM_BYTES {8}] [get_bd_cells QSFP/QSFP_$QSFP_INDEX/axi_width_converter_tx]
set_property -dict [list CONFIG.S_TDATA_NUM_BYTES.VALUE_SRC USER] [get_bd_cells QSFP/QSFP_$QSFP_INDEX/axi_width_converter_rx]
set_property -dict [list CONFIG.S_TDATA_NUM_BYTES {8} CONFIG.M_TDATA_NUM_BYTES {64}] [get_bd_cells QSFP/QSFP_$QSFP_INDEX/axi_width_converter_rx]
...
connect_bd_net [get_bd_pins QSFP/QSFP_$QSFP_INDEX/w56v0/dout] [get_bd_pins QSFP/QSFP_$QSFP_INDEX/eth/tx_preamblein_0]
