...
def read_VXLAN_count(eth):
    program_string = CONFIG_FILE
    program_string += ' -r --eth=' + str(eth) + ' --vxlan-count'
    try:
        output = subprocess.run(program_string, shell=True, capture_output=True, check=True)
        return output.stdout.decode('utf-8'), None
    except subprocess.SubprocessError as e:
...
def udp_config(network_id):
    if flask.request.method == 'PUT':
        ...
        for key in data:
            if key is not 'IP' and key is not 'Gateway' and key is not 'Netmask' and key is not 'MAC':
        ...
        if 'Netmask' in data:
            ...
            if failed:
                return str(failed), 404
        if 'MAC' in data:
            failed = write_UDP_gateway(network_id, data['MAC'])
            if failed:
                return str(failed), 404
    return_output = {}
...
def vxlan_config(network_id, vxlan_id):
    if flask.request.method == 'PUT':
        ...
        # Make sure the fields are correct
        for key in data:
            if key is not 'IP' and key is not 'Local Port' and key is not 'Remote Port' and key is not 'VNI':
                return 'Wrong data format', 404
